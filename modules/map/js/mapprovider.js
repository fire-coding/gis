...
var MapProvider = {
  ...
  scanexProvider: function(bounds) {
    return MapProvider.provide(bounds, "gis_ua_web", this);
  },

  kosmoProvider: function(bounds) {
    return MapProvider.provide(bounds, "gis_kosmo_ua", this);
  },

  provide: function(bounds, prefix, scope) {
    ...
    if (appMap.mapBounds.intersectsBounds(bounds) && z >= appMap.mapMinZoom && z <= appMap.mapMaxZoom) {
      return appMap.tilesUrl + prefix + "/" + z + "/" + x + "/" + y + "." + scope.type;
    } else {
