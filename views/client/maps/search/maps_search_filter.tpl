  ...
  <div class="module-wrapper">
    ...
    <form action="/maps_search" method="post">
      ...
      <table class="search_filter">
        <tr>
          <td>
            <a href="#" onclick="$('.filter-regions').attr('checked', 1);return false;">Виділити всі</a>
            &nbsp;/&nbsp;
            <a href="#" onclick="$('.filter-regions').attr('checked', null);return false;">Зняти всі</a>
          </td>
        </tr>
        {foreach from=$regions item=region}
          <tr>
            <td>
              <label for="region_{$region.CODEOBJ}"><input type="checkbox" checked  class="filter-regions" name="regions[{$region.CODEOBJ}]" id="region_{$region.CODEOBJ}" />{$region.TITLE_R}</label>
            </td>
          </tr>
        {/foreach}
